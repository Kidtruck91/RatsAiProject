...
def train_dqn():
    ...
    # Initialize DQN agents with parameters from config.py
    ...
    victories_player1, victories_player2, total_ties, total_game_lengths = 0, 0, 0, []

    for episode in range(EPISODES):
        game.reset_game()
        state1 = np.reshape(game.get_state(player1), [1, STATE_SIZE])
        state2 = np.reshape(game.get_state(player2), [1, STATE_SIZE])
        done, game_length = False, 0

        while not game.game_over:
            # Player 1 (Agent 1) takes an action
            action1 = agent1.choose_action(state1)
            action_str1 = ['draw', 'call_rats', 'peek_opponent', 'peek_self', 'swap_with_queen'][action1]
            ...
            if not done:
                ...
                action_str2 = ['draw', 'call_rats', 'peek_opponent', 'peek_self', 'swap_with_queen'][action2]
